...
export default function HagwonCard({ image, name, region, format, lessonType, ia_ee_tok, description, address, url }) {
  ...
  return (
      <div className="border rounded-4xl p-[1em] bg-black shadow-sm hover:shadow-lg transition-transform transform hover:-translate-y-1 flex flex-col gap-5 justify-between">
        <div className="flex flex-wrap justify-between flex-row">
          ...
          <div className="flex flex-wrap gap-2 w-[20em]">
            {[...format, ...lessonType].map((tag, i) => (
              <span
                key={i}
                className="text-xs px-2 py-1 bg-[#e9f3ff] text-[#216eb3] rounded-full font-medium"
              >
                {tag}
              </span>
            ))}
            {ia_ee_tok && ['IA', 'EE', 'TOK'].map((tag, i) => (
              <span
                key={`core-${i}`}
                className="text-xs px-2 py-1 bg-[#ffe9ff] text-[#b321b1] rounded-full font-medium"
              >
                {tag}
              </span>
            ))}
          ...
          <button
            className="toggle-details-btn border px-4 py-2 rounded-lg text-gray-600 hover:bg-gray-200 font-semibold text-sm"
        ...
        {showDetails && (
          <div className="border-t pt-4 text-gray-600 w-full">
            <p className="mb-4 text-sm">{description}</p>
            <p className="mb-4 text-sm">주소: {address}</p>
            <div className="flex gap-3 flex-wrap">
              ...
              <a href={url} target="_blank" rel="noopener noreferrer" className="contact-btn px-4 py-2 text-sm bg-gray-50 border border-gray-300 rounded-lg text-gray-900 hover:bg-gray-100 hover:border-gray-400">
                홈페이지
              </a>
            </div>
          </div>
