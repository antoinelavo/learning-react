...
export default function HagwonNewsletterPopup() {
  ...
  useEffect(() => {
    ...
    const isDismissed = localStorage.getItem('hagwonNewsletterDismissed');
    const hasSubscribed = localStorage.getItem('hagwonNewsletterSubscribed');
  ...
  async function handleSubscribe(e) {
    ...
    try {
      ...
      setStatus('success');
      localStorage.setItem('hagwonNewsletterSubscribed', 'true');
      ...
      console.error('Hagwon newsletter subscription error:', err);
      setErrorMessage('오류가 발생했습니다. 다시 시도해주세요.');
      setStatus('error');
    }
  }
  ...
  function handleDismiss() {
    setIsVisible(false);
    localStorage.setItem('hagwonNewsletterDismissed', Date.now().toString());
  }
              ...
              <div className="text-center py-2">
                <div className="flex items-center justify-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">새 학원 요청이 올라오면 이메일로 알려드릴게요.</p>
                  </div>
                </div>
              </div>
            ) : (
              <div className="flex flex-col sm:flex-row sm:items-center sm:gap-6 gap-3">
                ...
                <div className="flex-shrink-0 pr-8 sm:pr-0">
                  ...
                  <p className="text-sm text-gray-600">
                    하루에 한 번, 새로운 학원 요청글을 이메일로 알려드립니다.
                  </p>
                </div>
