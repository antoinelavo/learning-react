...
// Admin-only: every conversation on the platform, enriched with teacher and
// student display names for the admin conversations list. Relies on the
// admin_select RLS policy (users.role = 'admin') — returns nothing for a
// non-admin caller.
export async function getAllConversationsForAdmin() {
  ...
  const teacherUserIds = Array.from(new Set(conversations.map((c) => c.teacher_id)));
  const studentUserIds = Array.from(new Set(conversations.map((c) => c.student_id)));

  const [teachersRes, studentsRes] = await Promise.all([
    supabase.from('teachers').select('user_id, name').in('user_id', teacherUserIds),
    supabase.from('users').select('id, username, email').in('id', studentUserIds),
  ]);
  ...
  return conversations.map((c) => ({
    id: c.id,
    teacherName: teacherByUserId[c.teacher_id]?.name || '(알 수 없음)',
    studentName: studentDisplayName(studentByUserId[c.student_id] || {}),
    lastMessage: c.last_message_preview,
    lastMessageAt: c.last_message_at,
...
// Admin-only: a single conversation's teacher/student display names, for
// the conversation detail page's header. Same RLS reliance as above.
export async function getConversationForAdmin(conversationId) {
  ...
  const [{ data: teacher }, { data: student }] = await Promise.all([
    supabase.from('teachers').select('name').eq('user_id', c.teacher_id).maybeSingle(),
    supabase.from('users').select('username, email').eq('id', c.student_id).maybeSingle(),
  ]);

  return {
    id: c.id,
    teacherUserId: c.teacher_id,
    studentUserId: c.student_id,
