...
export async function activatePremium({
  ...
  durationMonths,
  expectedAmount,
  // Optional: the originating payment_request.id. When given, that row is
  // stamped admin_confirmed=true once activation succeeds (or turns out to
  // already have succeeded) — used by the Toss/NicePay routes (order.id)
  // and by the admin payments page for manually-confirmed bank transfers.
  paymentRequestId,
}) {
  async function markRequestConfirmed() {
    if (!paymentRequestId) return;
    await supabase
      .from('payment_request')
      .update({ admin_confirmed: true, confirmed_at: new Date().toISOString() })
...
// Reverses a single confirmation: removes exactly the successful_payments
// and teacher_premium rows that activatePremium() created for this
// payment_request (matched via payment_request_id, so a teacher's other
// premium purchases are untouched), then resets the request to pending.
// Rows confirmed before payment_request_id existed have no linked rows to
// remove — this still resets admin_confirmed, but any premium grant from
// that earlier confirmation is left in place (nothing to identify it by).
export async function deactivatePayment({ paymentRequestId }) {
  if (!paymentRequestId) {
    return { ok: false, error: 'missing_payment_request_id' };
  }
  ...
  if (premiumError) {
    return { ok: false, error: 'remove_premium_failed' };
  }
  ...
  const { error: paymentError } = await supabase
    ...
    .eq('payment_request_id', paymentRequestId);
  if (paymentError) {
    return { ok: false, error: 'remove_payment_record_failed' };
  }

  const { error: requestError } = await supabase
    .from('payment_request')
    .update({ admin_confirmed: false, confirmed_at: null })
    .eq('id', paymentRequestId);
  if (requestError) {
